...
class ReviewFeatureTransforming:
    ...
    def _review_feature_transforming(self, df):
        df = df.copy()
        df = df.drop_duplicates(subset=["reviewer_id", "product_id"], keep="last")
    ...
    def _review_time_feature(self, df):
        df = df.copy()
        df["review_time"] = pd.to_datetime(df["review_time"]).dt.strftime("%Y%m%d")
    ...
    def _mean_product_rating_feature_transforming(self, df):
        df = df.copy()
        mean_product_rating_df = (
            df.groupby("product_id")["product_rating"].mean().reset_index()
        )
    ...
    def _deviation_rating_product_feature_transforming(self, df):
        df = df.copy()
        df["deviation_rating_product"] = (
            df["product_rating"] - df["mean_product_rating"]
        )
        return df

    def _mean_review_rating_feature_transforming(self, df):
        df = df.copy()
        mean_review_rating_df = (
            df.groupby("reviewer_id")["product_rating"].mean().reset_index()
        )
    ...
    def _deviation_reviewer_rating_feature_transforming(self, df):
        df = df.copy()
        df["deviation_reviewer_rating"] = (
            df["product_rating"] - df["mean_reviewer_rating"]
        )
        return df
    ...
    def _total_products_rated_feature_transforming(self, df):
        df = df.copy()
        total_products_rated = (
            df.groupby("reviewer_id")["product_id"]
            .count()
            .reset_index()
    ...
    def _total_rating_users_feature_transforming(self, df):
        df = df.copy()
        total_rating_users = (
            df.groupby("product_id")["reviewer_id"]
            .nunique()
            .reset_index()
    ...
    def _review_day_to_last_reviewed_day_feature_transforming(self, df):
        df = df.copy()
        last_reviewed_day_df = (
            df.groupby("product_id")["review_time"].max().reset_index()
        )
    ...
    def _days_since_last_review_feature_transforming(self, df):
        ...
        df["days_since_last_review"] = (
            df["review_time"].max() - df["last_reviewed_day"]
        ).dt.days
        return df
...
class ProductFeatureTransforming:
    ...
    def _product_description_feature_transforming(self, df):
        ...
        df.loc[df["len_product_dscrp"] == 0, "product_description"] = "no_description"
        df.loc[
            (df["len_product_dscrp"] > 0) & (df["len_product_dscrp"] < 10),
            "product_description",
        ] = "sketchy"
        df.loc[
            (df["len_product_dscrp"] >= 10) & (df["len_product_dscrp"] < 20),
            "product_description",
        ] = "normal"
        df.loc[df["len_product_dscrp"] >= 20, "product_description"] = "detailed"
        return df

    def _title_cat_feature_transforming(self, df):
        df = df.copy()
        df["title_len"] = df["title"].apply(lambda x: len(x) if x is not None else 0)
        df.loc[df["title_len"] == 0, "title_cat"] = "no_title"
        df.loc[
            (df["title_len"] > 0) & (df["title_len"] < 100), "title_cat"
        ] = "sketchy_title"
        df.loc[
            (df["title_len"] >= 100) & (df["title_len"] < 200), "title_cat"
        ] = "normal_title"
        df.loc[
            (df["title_len"] >= 200) & (df["title_len"] < 500), "title_cat"
        ] = "detailed_title"
    ...
    def _category_feature_transforming(self, df):
        ...
        df[["rank", "category", "c", "d", "e", "f"]] = df["rank"].str.split(
            " in ", expand=True
        )
        df = df[
            [
...
class FeatureTransforming(
    ...
    def _feature_transforming(self, df):
        ...
        df["product_description"] = df["product_description"].fillna("no_description")
