...
class PredictionLayer(Layer):
    ...
    def build(self, input_shape):
        if self.task == "multiclass":
            if self.num_classes is None:
                raise ValueError("`num_classes` must be specified for 'multiclass' task.")
    ...
    def compute_output_shape(self, input_shape):
        if self.task == "binary":
            ...
            return (input_shape[0], self.num_classes)  # Multi-class output has shape (batch_size, num_classes)
        elif self.task == "regression":
            return (input_shape[0], 1)  # Regression output has shape (batch_size, 1)

    def get_config(self, ):
        config = {'task': self.task,
                  'num_classes': self.num_classes,  # 'num_classes': self.num_classes, 'use_bias': self.use_bias}
