...
def data_to_tensor_type(df, dense_cols, cat_cols):
    ...
    ds = tf.data.Dataset.zip((
        ...
        tf.data.Dataset.from_tensor_slices((
            tf.cast(tf.keras.utils.to_categorical(
                df['label'].values, num_classes=5), tf.float32)
        ))
...
class SecondOrderFeatureInteraction(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        dot_products = tf.matmul(
            concat_features, concat_features, transpose_b=True)
        ...
        flat_interactions = tf.reshape(tf.boolean_mask(
            dot_products, mask), (batch_size, out_dim))
...
class DLRM(tf.keras.Model):
    def __init__(
        ...
        self.emb = [tf.keras.layers.Embedding(
            size, embedding_dim) for size in embedding_sizes]
...
def build_and_train_model(config):
    ...
    ds_train = data_to_tensor_type(
        df_train, dense_cols=dense_cols, cat_cols=cat_cols)
    ds_valid = data_to_tensor_type(
        df_val, dense_cols=dense_cols, cat_cols=cat_cols)
    ds_test = data_to_tensor_type(
        df_test, dense_cols=dense_cols, cat_cols=cat_cols)
