...
columns = ['original_label', *(f'I{i}' for i in range(1, 18)), *(f'C{i}' for i in range(1, 8))]
...
class TQDMCallback(Callback):
    def on_epoch_begin(self, epoch, logs=None):
        self.progress_bar = tqdm.tqdm(total=self.params['steps'], desc=f'Epoch {epoch + 1}', unit='batch')
...
model = DeepFEFM(linear_feature_columns, dnn_feature_columns, task='multiclass', num_classes=5)
...
true_labels = tf.argmax(to_categorical(test[target].values, num_classes=5), axis=1).numpy()
...
test_loss, test_accuracy = model.evaluate(
    test_model_input,
    to_categorical(test[target].values, num_classes=5),  # Ensure labels are one-hot encoded
    batch_size=256
)
